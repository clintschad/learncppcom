...
* unit test - unit being a function or class
* integration test - after unit has been tested, test system after integrating unit into main program

### 9.2 — Code coverage
* Statement coverage - what percentage of the total number of statements do the tests cover?
* Branch coverage - for `if` and `switch` statements, how many of the possible branches do the tests cover?
    - Aim for 100% branch coverage.
* `for` loop coverage - check for `k` = 0, 1, and 2. My note: check for last case too.
* Function input parameter testing:
    - Integer inputs: test with a normal value, then 0, and a negative number. Can also check for overflow if relevant (operations the increase number, such as add and multiply). Can check for underflow with operations that decrease number, such as subtraction and division.
    - Floating point inputs: verify function can handle precision issues. For `double`, test with values from 0.1 to -0.1 to test values that are slightly larger than expected. Also test with 0.7 to -0.7 to test numbers that are slightly smaller than expected. _Why these numbers?_
    - String inputs: test for empty, alphanumeric, whitespace, and strings that are all whitespace.
    - Pointer input: include test for `nullptr` (hasn't been covered yet)

### 9.3 — Common semantic errors in C++

* For floating types, verify the type has enough precision (decimal places) for accepting values with that precision or being able to hold results from operations that affect precision. The number of arithmetic operations on a float increases its rounding error.
* Be aware that integer division leads to a rounded integer result. Cast one of the operands to get the result in a float if desired.
* Be careful to use  `==` instead of `=` in `if` statements. Most compilers should throw a warning when `=` is an `if` statement.
* Be careful to include the parentheses for function calls, e.g. use `myfunc()` instead of `myfunc`. Using only the function's name causes the function's address to be used instead.

### 9.4 — Detecting and handling errors
* Use the `halt` statement or `std::exit()` for fatal errors, e.g. program cannot or should not continue running.
* When to use `std::cout` vs `std::err`:
    - `cout` for text the user will see during and after normal program operation. Use `err` for writing to a logfile for status or diagnostic information.

### 9.5 — std::cin and handling invalid input
* EOF (End Of File) can also be generated by `std::cin`. This can be triggered by the user via special key commands that vary by OS.

### 9.6 — Assert and static_assert
* The `assert` function is triggered when the condition inside is false. An error message will be displayed via `std::abort`. Message usually contains the expression that failed, name of code file, and line number.
    - A string can be added to `assert` parameter to give more description, e.g. `assert(found && "Car could not be found in database");`
* `assert` is checked at run time.
* The `static_assert` is checked at compile time.
